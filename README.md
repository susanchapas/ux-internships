...
├── targets.json         # 416 companies across 17 categories
...
`targets.json` holds 416 companies across 17 categories, plus 19 government
...
| `ux_agencies` | 1 | 31 | Design studios and consultancies |
| `civic_tech` | 1 | 15 | Government contractors. Heavy UX research. |
| `accessibility` | 1 | 10 | Continuous with your smart-glasses lab work |
| `ux_research_authorities` | 1 | 10 | NN/g, Baymard, research-tool vendors. Mostly no ATS — see note below. |
...
| `fortune500_nycnj` | 2 | 35 | NY/NJ Fortune 500 not covered elsewhere |
| `best_places_to_work` | 2 | 70 | Glassdoor/Fortune/LinkedIn lists, NYC/NJ. Wide net. |
| `edtech` | 3 | 12 | |
| `quant_trading` | 3 | 12 | Few design roles, exceptional pay |

### A note on `ux_research_authorities`

Highest signal per application on the whole list, and the lowest resolve rate.
Nielsen Norman Group, Baymard, Rosenfeld Media, IxDF, and the Center for Civic
Design are 15-50 person outfits with no standard ATS and no structured summer
...
`python resolve.py --category ux_research_authorities` from a network that can
reach the ATS APIs before reading anything into an empty result here.
